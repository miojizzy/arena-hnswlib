...
本项目集中管理所有配置文件（dotfiles），涵盖开发环境、CI/CD 流水线、AI 助手等配置。
...
使用 [.devcontainer/devcontainer.json](./.devcontainer/devcontainer.json) 构建开发环境：
...
- **配置文件**: [.cnb.yml](./.cnb.yml) 定义开发环境构建和服务配置（CNB 平台不支持软链接配置文件）
...
2. 自动使用 `.devcontainer/devcontainer.json` 构建环境

### 在 CNB Workspace 中开发

1. 创建 CNB Workspace，关联项目仓库
2. 自动使用 `.cnb.yml` 和 `.ide/Dockerfile` 构建环境
3. 详细配置参见 [CNB_DOCKER_README.md](./CNB_DOCKER_README.md)

### 修改 AI 助手配置

在 `.github/` 下修改，CNB 环境会自动同步：
